...
import swz.infra.tools.cache.memcached.client.SockIOPool;
...
public class MemcachedCacheManager implements ICacheManager<IMemcachedCache> {
	 ...
	 * (non-Javadoc)
	 * 
	 * @see com.olymtech.common.cache.ICacheManager#start()
	 */
	public void start() {
		...
		if (memcachedClientconfigs != null && memcachedClientconfigs.size() > 0 && memcachedClientSocketPoolConfigs != null
				&& memcachedClientSocketPoolConfigs.size() > 0) {
			try {
		...
		try {
			if (supportMultiConfig) {
				...
				while (urls.hasMoreElements()) {
					...
					CacheUtil.loadMemcachedConfigFromURL(url, factory, memcachedClientconfigs, memcachedClientSocketPoolConfigs, memcachedClientClusterConfigs);

					logger.info(new StringBuilder().append("load config from :").append(url.getFile()));
				}
			} else {
				URL url = null;
				ClassLoader loader = this.getClass().getClassLoader();//Thread.currentThread().getContextClassLoader();
	...
	protected void initMemCacheClientPool() {
		...
		for (MemcachedClientSocketPoolConfig socketPool : memcachedClientSocketPoolConfigs) {
			if (socketPool != null && socketPool.getServers() != null && !socketPool.getServers().equals("")) {
		...
		for (MemcachedClientConfig node : memcachedClientconfigs) {
			...
			try {
				...
				logger.error(new StringBuilder().append("Not find class name:").append(node.getErrorHandler()).append("please check space char or tab char"));
			}
	 ...
	 * (non-Javadoc)
	 * 
	 * @see com.olymtech.common.cache.ICacheManager#stop()
	 */
	public void stop() {
