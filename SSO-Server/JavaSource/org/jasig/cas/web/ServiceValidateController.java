 ...
 * @version $Revision: 1.4.2.5 $ $Date: 2011/04/14 11:38:40 $
 * @since 3.0
 */
public class ServiceValidateController extends AbstractController {
    ...
    private static final String T_G_T = "tgt";
	...
	if (request.getParameter("webServerIPandPort") != null)
	    MemCachedFactory.getCache().put(serviceTicketId + "forlogout", request.getParameter("webServerIPandPort"));
	
	//System.out.println("webServerIPandPort:"+request.getParameter("webServerIPandPort"));
	...
	try {
	    ...
	    //System.out.println(this.centralAuthenticationService.getTicketGrantingTicketId(serviceTicketId));
	    ...
	    success.addObject(T_G_T, this.centralAuthenticationService.getTicketGrantingTicketId(serviceTicketId));
	    
	    //for .net logout
	    MemCachedFactory.getCache().put(serviceTicketId + ".net", serviceTicketId);
	    try {
		Authentication auth = (Authentication)(assertion.getChainedAuthentications().get(0));
		MemCachedFactory.getCache().put(serviceTicketId + ".logoutUserName", auth.getPrincipal().getId());
	    }catch (Exception e) {
		//e.printStackTrace();
	    }
