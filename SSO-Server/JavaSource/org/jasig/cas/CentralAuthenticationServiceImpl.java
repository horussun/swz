...
public final class CentralAuthenticationServiceImpl implements CentralAuthenticationService {
	...
	/**
	 * New Ticket Registry for storing and retrieving services tickets. Can
	 * point to the same one as the ticketRegistry variable.
	 */
	@NotNull
	...
	/**
	 * @throws IllegalArgumentException
	 *             if TicketGrantingTicket ID, Credentials or Service are null.
	 */
	...
	public String grantServiceTicket(final String ticketGrantingTicketId, final Service service, final Credentials credentials)
		...
		if (credentials != null) {
			try {
				...
				if (!(authentication.getPrincipal().equals(originalAuthentication.getPrincipal()) && authentication.getAttributes()
						.equals(originalAuthentication.getAttributes()))) {
		...
		// XXX fix this
		final UniqueTicketIdGenerator serviceTicketUniqueTicketIdGenerator = this.uniqueTicketIdGeneratorsForService.get(service
				.getClass().getName());

		final ServiceTicket serviceTicket = ticketGrantingTicket.grantServiceTicket(serviceTicketUniqueTicketIdGenerator
				.getNewTicketId(ServiceTicket.PREFIX), service, this.serviceTicketExpirationPolicy, credentials != null);
	...
	public String delegateTicketGrantingTicket(final String serviceTicketId, final Credentials credentials) throws TicketException {
		...
		try {
			...
			if (registeredService == null || !registeredService.isEnabled() || !registeredService.isAllowedToProxy()) {
				log.warn("ServiceManagement: Service Attempted to Proxy, but is not allowed.  Service: ["
	...
	/**
	 * @throws IllegalArgumentException
	 *             if the ServiceTicketId or the Service are null.
	 */
	...
	public Assertion validateServiceTicket(final String serviceTicketId, final Service service) throws TicketException {
		...
		setTgt(serviceTicket.getGrantingTicket().getId());
		final RegisteredService registeredService = this.servicesManager.findServiceBy(service);

		if (registeredService == null || !registeredService.isEnabled()) {
			log.warn("ServiceManagement: Service does not exist is not enabled, and thus not allowed to validate tickets.   Service: ["
					+ service.getId() + "]");
		...
		try {
			synchronized (serviceTicket) {
				...
				if (!serviceTicket.isValidFor(service)) {
					...
					//log.error("ServiceTicket [" + serviceTicketId + "] with service [" + serviceTicket.getService().getId()
							//+ " does not match supplied service [" + service + "]");
					// throw new TicketValidationException(serviceTicket.getService());
				}
			}

			final int authenticationChainSize = serviceTicket.getGrantingTicket().getChainedAuthentications().size();
			final Authentication authentication = serviceTicket.getGrantingTicket().getChainedAuthentications().get(
					authenticationChainSize - 1);
			final Principal principal = authentication.getPrincipal();
			final String principalId = registeredService.isAnonymousAccess() ? this.persistentIdGenerator.generate(principal,
					serviceTicket.getService()) : principal.getId();
			...
			if (!registeredService.isIgnoreAttributes()) {
				...
				final MutableAuthentication mutableAuthentication = new MutableAuthentication(modifiedPrincipal, authentication
	 ...
	 *            The ticketGrantingTicketExpirationPolicy to set.
	 */
	 ...
	 *            The uniqueTicketIdGenerator to use
	 */
	 ...
	 *            The serviceTicketExpirationPolicy to set.
	 */
	...
	@Override
	public String getTicketGrantingTicketId(String serviceTicketId) {
