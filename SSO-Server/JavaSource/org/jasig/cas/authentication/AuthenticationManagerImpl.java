 ...
 * </p>
 * <ul>
 * <li> <code>authenticationHandlers</code> - The array of AuthenticationHandlers
 * that know how to process the credentials provided.
 ...
 * @version $Revision: 1.1 $ $Date: 2009-12-14 23:33:36 -0500 (Mon, 14 Dec
 *          2009) $
 * @since 3.0
...
public final class AuthenticationManagerImpl extends AbstractAuthenticationManager {
			...
			if (authenticationHandler.supports(credentials)) {
				...
				if (!authenticationHandler.authenticate(credentials)) {
					if (log.isInfoEnabled()) {
						log.info("AuthenticationHandler: " + authenticationHandler.getClass().getName()
								+ " failed to authenticate the user which provided the following credentials: " + credentials.toString());
					}
				} else {
					if (log.isInfoEnabled()) {
						log.info("AuthenticationHandler: " + authenticationHandler.getClass().getName()
	...
	/**
	 * @param credentialsToPrincipalResolvers
	 *            The credentialsToPrincipalResolvers to set.
	 */
