 ...
 * @version $Revision: 1.1.2.4 $ $Date: 2011/04/14 11:38:27 $
 * @since 3.1
 * 
 */
public abstract class AbstractWebApplicationService implements WebApplicationService {
	...
	final String logoutRequest = "<samlp:LogoutRequest xmlns:samlp=\"urn:oasis:names:tc:SAML:2.0:protocol\" ID=\"" + GENERATOR.getNewTicketId("LR")
		+ "\" Version=\"2.0\" IssueInstant=\"" + SamlUtils.getCurrentDateAndTime()
		+ "\"><saml:NameID xmlns:saml=\"urn:oasis:names:tc:SAML:2.0:assertion\">@NOT_USED@</saml:NameID><samlp:SessionIndex>" + sessionIdentifier
		+ "</samlp:SessionIndex></samlp:LogoutRequest>";

	this.loggedOutAlready = true;
	
	try {
            String logoutUserName = MemCachedFactory.getCache().get(sessionIdentifier+".logoutUserName")==null?null:(String)MemCachedFactory.getCache().get(sessionIdentifier+".logoutUserName");
            if(logoutUserName != null) {
                //new AppAccessLogManager().doUpdatePlus(BaseDBOperate.getConnection(), logoutUserName);
                MemCachedFactory.getCache().remove(sessionIdentifier + ".logoutUserName");
            }
        }catch(Exception e) {
            //e.printStackTrace();
        }
	...
	String logoutUrl = MemCachedFactory.getCache().get(sessionIdentifier + "forlogout") == null ? null : (String) MemCachedFactory.getCache().get(
		sessionIdentifier + "forlogout");
	MemCachedFactory.getCache().remove(sessionIdentifier + ".net");
	if (logoutUrl != null) {
	    MemCachedFactory.getCache().remove(sessionIdentifier + "forlogout");
	    String yy = url.substring(url.indexOf("//") + 2, url.length());
	    String zz = yy.substring(yy.indexOf("/"), yy.length());
	    url = logoutUrl + zz;
	}

	if (this.httpClient != null) {
	    return this.httpClient.sendMessageToEndPoint(url, logoutRequest, true);
	}
