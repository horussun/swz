...
public class StringUtil {
	...
	public static String getFilledString(int length, String str) {
		String buff = "";
	...
	public static String getFixedLengthStringPrefix(String str, int length,
			String filler) {
		int strlen = str.length();
		if (strlen > length) {
			return str.substring(0, length);
		}
		return getFilledString(length - strlen, filler) + str;
	}

	public static String getFixedLengthStringSuffix(String str, int length,
			String filler) {
		int strlen = str.length();
		if (strlen > length) {
			return str.substring(0, length);
		}
		return str + getFilledString(length - strlen, filler);
	}

	public static String number_editing(String numerical_value, int small_figure) {
		...
		StringTokenizer st = new StringTokenizer((df.format(num.doubleValue()))
				.toString(), ".");
	...
	public static double number_editing(double value, int small_figure) {
		String valueStr = String.valueOf(value);
		String retValue = number_editing(valueStr, small_figure);
		return Double.parseDouble(retValue);
	}

	public static String haihun_editing(String str, int point) {
		return str.substring(0, point) + "-"
				+ str.substring(point, str.length());
	}

	public static String urlToScreen(String str) {
		return str.substring(1).toUpperCase();
	}

	public static String transpose_blank(Date transcribe) {
		if (transcribe == null) {
			return "";
		}
		String value = transChina(transcribe.toString());
		return value;
	}

	public static String transpose_blank(String transcribe) {
		if (transcribe == null) {
			transcribe = "";
		}
		return transcribe;
	...
	public static String transBlankAndToChina(String transcribe) {
		if (transcribe == null) {
			transcribe = "";
		}
		String value = transChina(transcribe);
		return value;
	}

	public static String transUTF8ToGBK(String transcribe) {
		if (transcribe == null) {
			transcribe = "";
		}
		try {
			String retStr = new String(transcribe.getBytes("UTF-8"), "utf-8");
			return retStr;
		} catch (Exception e) {
			return "";
		}
	...
	public static String transpose_blank(int v) {
		String value = transChina(String.valueOf(v));
		return value;
	}

	public static String transpose_blank(double v) {
		String value = transChina(String.valueOf(v));
		return value;
	}

	public static String transpose_blankE(String transcribe) {
		if (transcribe == null) {
			transcribe = "";
		}
		return transcribe;
	...
	public static String transpose_space(String transpace) {
		if (transpace == null) {
			transpace = "丂";
		}
		return transpace;
	...
	public static String transpose_null(String transcribe) {
		return (transcribe == null || transcribe.equals("")) ? null
				: transcribe;
	}

	public static String transpose_HTMLblank(String str) {
	...
	public static BigDecimal divideThousand(BigDecimal input) {
		return input.divide(new BigDecimal("1000"), BigDecimal.ROUND_HALF_UP);
	}

	public static int divideThousand(int input) {
		BigDecimal decimal = new BigDecimal("" + input);
		return divideThousand(decimal).intValue();
	}
	...
	public static String convertString(String data) {
		...
		for (int i = 0; i < src.length(); i++) {
			int idx;
			while (-1 != (idx = data.indexOf(src.charAt(i)))) {
				data = data.substring(0, idx) + dst.substring(i, i + 1)
	...
	public static String convertTwoSpace(String val) {
	...
	public static String getLaterString(String str, String separator) {
		if (str == null)
			str = "";
	...
	public static String getPrefix(String str, String separator, int flag) {
		if (str == null)
			str = "";
	...
	/**
	 * 功能：得到数据库系统时间 返回： String
	 */
	public static Timestamp getDataBaseNowTime() {
		// String sqlStr="select to_char(sysdate,'yyyy-mm-dd hh24:mi:ss') as
		// time from dual";
		...
		// Timestamp now=null;
		// try {
		// conn=DBConnectionManager.getInstance().getConnection(Config.ConfigPoolName);
		// stmt = conn.prepareStatement(sqlStr);
		// rs = stmt.executeQuery();
		// if (rs.next()) {
		// now=rs.getTimestamp("time");
		// }
		// } catch (Exception e) {
		//			
		// } finally {
		// stmtRsClose(stmt, rs);
		// DBConnectionManager.getInstance().freeConnection(Config.ConfigPoolName,
		// conn);
		// }
	...
	/**
	 * 功能：转换成Float 参数： String 返回： Float
	 */
	...
	/**
	 * 功能：把特殊字符转换成HTML格式的字符 参数： String 返回： String
	 */
	...
	/**
	 * 功能：得到最大长度的字符串,如果不到长度,就取当前字符串作为返回. 参数： int 返回： String
	 */
	public static String maxLenString(String str, int len) {
		str = transpose_blank(str);
	 ...
	 * @return
	 */
	public static String formatValue(String value, String type, int length) {
		String javaType = getDataType(type);
		String retValue = value;
		if (javaType.equals("String")) {
			if (value != null && value.length() > length) {
				retValue = value.substring(0, length);
			} else {
				retValue = StringUtil.transpose_blank(value);
			}
		} else if (javaType.equals("int")) {

		} else if (javaType.equals("Date")) {

			// retValue=StringUtil.formatDate(value);

		} else if (javaType.equals("double")) {
			retValue = StringUtil.number_editing(value, 2);
		}
		return retValue;
	}

	/**
	 * 根据数据库(SQL-SERVER 2K)的数据类型取得JAVA数据类型
	 * 
	 * @param dataType
	 * @return
	 */
     ...
     * Steven Si 20060622
     * */
    public static String getRandomStr(int length) throws InterruptedException{
        String content="";
        for(int i=0;i<length;i++){
            content+=getChar();
	 ...
	 * @return
	 */
	public static boolean firstDateAfterSecDate(Date oneDate, Date twoDate) {
		Calendar oneCa = Calendar.getInstance();
		oneCa.setTime(oneDate);
		Calendar twoCa = Calendar.getInstance();
		twoCa.setTime(twoDate);
		return oneCa.after(twoCa);
	}

	/**
	 * 获取随机字符 Steven Si 20060622
	 */
	public static char getChar() {
		Random random = new Random();
		char ch = '0';
		LinkedList ls = new LinkedList();
		for (int i = 2; i < 10; i++) {// 0-9
	...
	/**
	 * 验证日期、时间格式为输入的pattern型
	 * @param datetime pattern
	 * @return
	 */
	
	public static boolean timeFormatValidator(String datetime,String pattern){
		
		boolean result=true;		
		SimpleDateFormat dateValidator=new  SimpleDateFormat(pattern);  
		dateValidator.setLenient(false);//这个的功能是不把1996-13-3 转换为1997-1-3
	    try{  
	       dateValidator.parse(datetime);  
	    }
	    catch(Exception  e)
	    {  
	    	return false; 
