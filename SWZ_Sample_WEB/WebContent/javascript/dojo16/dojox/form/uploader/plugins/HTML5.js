...
dojo.declare("dojox.form.uploader.plugins.HTML5", [], {
	...
	uploadWithFormData: function(/* Object */data){
		...
		if(data instanceof FileList){
			dojo.forEach(data, function(f, i){
...
//				var fileName  = this.inputNode.files[i].fileName;
				var orgName=data[i].fileName;
				if (orgName==null)orgName=data[i].name;
				var fileName="fileName"+orgName.substring(orgName.lastIndexOf("."));
				var binary;

				try{
					binary = data[i].getAsBinary() + EOL;
					part += boundary + EOL;
			...
			if(filesInError.length){
				if(filesInError.length >= data.length){
			...
			dojo.forEach(this.inputNode.files, function(f, i){
				var fieldName = "uploadedfiles[]";//+i;
//				var fileName  = this.inputNode.files[i].fileName;
				var orgName=this.inputNode.files[i].fileName;
				var fileName="fileName"+orgName.substring(orgName.lastIndexOf("."));
				var binary;
