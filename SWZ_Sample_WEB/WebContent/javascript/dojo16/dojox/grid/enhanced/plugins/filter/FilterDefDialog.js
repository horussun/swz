...
dojo.declare("dojox.grid.enhanced.plugins.filter.FilterDefDialog", null, {
	...
	onFilter: function(e){
		...
		if(this.canFilter()){
			...
			//modified by lfp 2011-12-21
			
			if(this.plugin.grid.selectedStore){
				this.plugin.grid.selectedStore();
			}
			this.plugin.grid.store.url=window.actionPath + "/search/doFilter";
			this.plugin.grid.query={
					name:this.plugin.grid.name
			};
			dojo.cookie(window.userId+"_"+this.plugin.grid.name+"_ft", dojo.toJson(this._savedCriterias), {
		            expires: 7
		        });
			//console.log(this.plugin.grid.name+"_fr");
			dojo.cookie(window.userId+"_"+this.plugin.grid.name+"_fr", this._relOpCls, {
	            expires: 7
	        });
			console.log("save the _savedCriterias to cookie");
			//moidfied by lfp 2011-12-21 end
	...
	onClearFilter: function(e){
		...
		dojo.cookie(window.userId+"_"+this.plugin.grid.name+"_ft", null, {
            expires: -1
        });
		dojo.cookie(window.userId+"_"+this.plugin.grid.name+"_fr", null, {
	        expires: -1
	    });
		console.log("delete the _savedCriterias from cookie");
		//moidfied by lfp 2011-12-21 end
	},
	onCancel: function(e){
		// summary:
